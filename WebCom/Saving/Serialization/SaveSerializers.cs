...
public abstract class SaveSerializers : Serializers<SaveSerializer>
{
  internal class DynamicSaveSerializers : SaveSerializers
  {
    private SaveSerializer CreateSerializer(Type type)
    {
      ...
      {
        if (!data.TryGet(member.Member.Name, out object value))
        {
          return null;
        }

        // Patch to keep old worlds working with the copy to absoluteAquarian's GUID serializer.
        if (type == typeof(Guid) && value is string str)
        {
          return Guid.Parse(str);
        }

        return TagIO.Deserialize(type, value);
      }
